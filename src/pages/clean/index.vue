    ...
    <div class="space-y-6">
      <section class="rounded-3xl border border-slate-200/70 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p class="text-sm font-semibold text-emerald-600 dark:text-emerald-300">{{ $t('pages.clean.sectionLabel') }}</p>
            <h2 class="mt-2 text-2xl font-bold">{{ $t('pages.clean.title') }}</h2>
          </div>
        </div>
        <!-- الفلترة -->
      ...
      <div class="hidden overflow-x-auto rounded-3xl border border-slate-200/70 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:block">
        <table class="min-w-full text-sm text-right">
          <thead class="bg-emerald-600 text-white">
            <tr>
              <th class="px-4 py-3">{{ $t('pages.clean.table.date') }}</th>
              <th class="px-4 py-3">{{ $t('pages.clean.table.startTime') }}</th>
              <th class="px-4 py-3">{{ $t('pages.clean.table.endTime') }}</th>
              <th class="px-4 py-3">{{ $t('pages.clean.table.type') }}</th>
              <th class="px-4 py-3">{{ $t('pages.clean.table.status') }}</th>
              <th class="px-4 py-3">{{ $t('pages.clean.table.responsible') }}</th>
            </tr>
          </thead>
          ...
          <div class="flex items-start justify-between gap-4">
            <div>
              ...
              <div class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                {{ $t('pages.clean.mobileSummary', { type: $t(item.type), responsible: $t(item.responsible) }) }}
              </div>
            </div>
...
const data = ref<CleanRow[]>([
  {
    date: '2025-10-10',
    start: '09:00',
    end: '11:30',
    type: 'pages.clean.types.propertyWipe',
    status: 'common.status.scheduled',
    responsible: 'pages.clean.responsible.ahmedAli'
  },
  {
    date: '2025-10-12',
    start: '14:00',
    end: '16:00',
    type: 'pages.clean.types.propertySweep',
    status: 'common.status.completed',
    responsible: 'pages.clean.responsible.monaHussein'
  },
  {
    date: '2025-10-15',
    start: '10:00',
    end: '12:00',
    type: 'pages.clean.types.elevatorWipe',
    status: 'common.status.postponed',
    responsible: 'pages.clean.responsible.khaledMostafa'
  }
]);
...
const statusColor = (status: StatusKey) => {
  return {
    'common.status.scheduled': 'bg-yellow-100 text-yellow-800',
    'common.status.completed': 'bg-green-100 text-green-800',
    'common.status.postponed': 'bg-red-100 text-red-800'
  }[status];
};
</script>
